...
/**
  ...
  The letructor takes three arguments:
    ...
    for (let name in inspectedProps) {
      ...
      switch (type) {
        ...
        case 'number': value = parseFloat(value)
          break
        case 'arrayofnumber': value = this.convertToArrayOfNumber(value)
          break
        case 'point': value = go.Point.parse(value)
          break
        case 'size': value = go.Size.parse(value)
          break
        case 'rect': value = go.Rect.parse(value)
          break
        case 'spot': value = go.Spot.parse(value)
          break
        case 'margin': value = go.Margin.parse(value)
          break
        case 'checkbox': value = input.checked
          break
      ...
      if (change) { // only change properties it needs to change instead all of them
        for (let i = 0; i < diagram.selection.count; i++) {
          ...
          if (data) { // ignores the selected node if there is no data
            ...
            // convert to specific type, if needed
            switch (type) {
              case 'boolean': value = !(value === false || value === 'false' || value === '0')
                break
              case 'number': value = parseFloat(value)
                break
              case 'arrayofnumber': value = this.convertToArrayOfNumber(value)
